...
| 有効テーマの CSS ファイル直接編集 / `shared/styles/slide.css` の編集 | `/design-tune` で出力先にも反映（`slide.css` 含む） |
...
`design.config.yaml` のトークン変更を CSS に反映する。theme ごとの `slide.css` と `shared/styles/slide.css` の変更も出力先にコピーする。HTML の再生成は行わない。
...
- `shared/styles/slide.css` — 全 theme 共通のレイアウト/コンポーネント CSS
- `themes/<name>/styles/` — テーマごとの CSS トークン（primitives → semantic → component）+ theme override
