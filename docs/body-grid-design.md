...
- `body-2col` / `body-3col` / `body-grid` を内部では同じ grid layout engine に統一する
...
将来、header を軽くする grid 専用 template を別途足す余地はあるが、V1 には含めない。
...
- header なし or 軽量 header の grid template
