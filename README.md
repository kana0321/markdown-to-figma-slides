...
Git に不慣れな場合は、[Git の最低限運用](docs/git-minimum-workflow.md) を先に読むと進めやすくなります。
...
10 種類のテンプレートが用意されています。`cover` / `section` / `agenda` は見出しレベルから自動選択されます。`###` のスライドには `<!-- slide: template=... -->` で明示指定できます。
