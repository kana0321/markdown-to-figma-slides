...
スライドの見た目は `theme` として切り替えられます。テーマは `themes/<name>/` 配下にまとまり、CSS、テンプレート、既定値、フォント読み込みを持ちます。
...
テーマを切り替えたあとは、HTML を再生成してください。

```bash
./scripts/run_pipeline.sh --project-root . --input input/raw/source.md
```

初期状態の `design.config.yaml` は最小構成です。ベースの見た目はテーマ既定値で決まり、`design.config.yaml` には project 固有の上書きだけを追加していく想定です。
...
11 種類のテンプレートが用意されています。`cover` / `section` / `agenda` は見出しレベルから自動選択されます。`###` のスライドには `<!-- slide: template=... -->` で明示指定できます。
...
| `body-grid` | strict な grid comment で組む自由レイアウト |
| `body-grid-full` | header なしでコンテンツを大きく見せる grid レイアウト |
...
`body-grid` は `<!-- grid: ... -->` と `<!-- cell: ... -->` を使って、セル単位で配置を定義します。
...
- `grid`: `columns`, `rows`, `gap`, `col_gap`, `row_gap`
- `cell`: `col`, `row`, `col_span`, `row_span`

`gap` / `col_gap` / `row_gap` の値は `sm` / `md` / `lg` です。

### Body Grid Full

`body-grid-full` は `body-grid` と同じ strict grammar を使いながら、標準の body header を描画しない variant です。
セクション見出しだけで文脈が足りていて、画像やカードなどのコンテンツ自体を大きく見せたい時に使います。
...
<!-- cell: col=1; row=1 -->
![alt](https://images.unsplash.com/example?w=1200&h=675&fit=crop&q=80)
<!-- /cell -->
