...
生成されるプロジェクトはバージョン付きスナップショット (`v1`, `v2`, ...) を持ち、theme を切り替えながら運用できます。  
この Skill の仕様や運用ルールは `SKILL.md` のみに完結していません。  
`skills/references/workflow.md` などの補助ドキュメントを含めて全体で構成されています。  
そのため、利用する際は `SKILL.md` 単体ではなく、関連ファイル一式をまとめてダウンロードし、元のフォルダ構成を維持したまま使用してください。
...
初期化直後の project template にはサンプルの branding ロゴ設定も入っているため、
cover / end と body / agenda footer のロゴ表示もそのまま確認できます。
...
project template は初期状態で、同梱の `input/raw/images/logo-horizontal-light.svg` /
`logo-horizontal-dark.svg` を branding ロゴとして参照します。
ロゴを隠したい場合は `design.config.yaml` で `cover_logo_enabled: false` または
`footer_logo_enabled: false` を指定し、差し替えたい場合は `branding.cover_logo` /
`branding.footer_logo` の `light_src` / `dark_src` を更新してください。
